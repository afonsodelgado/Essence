...
suite('Configuration Resolver Service', () => {
	...
	setup(() => {
		...
		configurationResolverService = new ConfigurationResolverService(uri.parse('file:///Essence/workspaceLocation'), envVariables, editorService, TestEnvironmentService, new TestConfigurationService(), mockCommandService);
	});
	...
	test('substitute one', () => {
		if (platform.isWindows) {
			assert.strictEqual(configurationResolverService.resolve('abc ${workspaceRoot} xyz'), 'abc \\Essence\\workspaceLocation xyz');
		} else {
			assert.strictEqual(configurationResolverService.resolve('abc ${workspaceRoot} xyz'), 'abc /Essence/workspaceLocation xyz');
		}
	});
	...
	test('substitute many', () => {
		if (platform.isWindows) {
			assert.strictEqual(configurationResolverService.resolve('${workspaceRoot} - ${workspaceRoot}'), '\\Essence\\workspaceLocation - \\Essence\\workspaceLocation');
		} else {
			assert.strictEqual(configurationResolverService.resolve('${workspaceRoot} - ${workspaceRoot}'), '/Essence/workspaceLocation - /Essence/workspaceLocation');
		}
	});
	...
	test('substitute one env variable', () => {
		if (platform.isWindows) {
			assert.strictEqual(configurationResolverService.resolve('abc ${workspaceRoot} ${env:key1} xyz'), 'abc \\Essence\\workspaceLocation Value for Key1 xyz');
		} else {
			assert.strictEqual(configurationResolverService.resolve('abc ${workspaceRoot} ${env:key1} xyz'), 'abc /Essence/workspaceLocation Value for Key1 xyz');
		}
	});
	...
	test('substitute many env variable', () => {
		if (platform.isWindows) {
			assert.strictEqual(configurationResolverService.resolve('${workspaceRoot} - ${workspaceRoot} ${env:key1} - ${env:key2}'), '\\Essence\\workspaceLocation - \\Essence\\workspaceLocation Value for Key1 - Value for Key2');
		} else {
			assert.strictEqual(configurationResolverService.resolve('${workspaceRoot} - ${workspaceRoot} ${env:key1} - ${env:key2}'), '/Essence/workspaceLocation - /Essence/workspaceLocation Value for Key1 - Value for Key2');
		}
	});
	...
	test('substitute one configuration variable', () => {
		...
		let service = new ConfigurationResolverService(uri.parse('file:///Essence/workspaceLocation'), envVariables, new TestEditorService(), TestEnvironmentService, configurationService, mockCommandService);
	...
	test('substitute nested configuration variables', () => {
		...
		let service = new ConfigurationResolverService(uri.parse('file:///Essence/workspaceLocation'), envVariables, new TestEditorService(), TestEnvironmentService, configurationService, mockCommandService);
		if (platform.isWindows) {
			assert.strictEqual(service.resolve('abc ${config:editor.fontFamily} ${config:terminal.integrated.fontFamily} xyz'), 'abc foo \\Essence\\workspaceLocation bar bar xyz');
		} else {
			assert.strictEqual(service.resolve('abc ${config:editor.fontFamily} ${config:terminal.integrated.fontFamily} xyz'), 'abc foo /Essence/workspaceLocation bar bar xyz');
		}
	});
	...
	test('substitute one env variable and a configuration variable', () => {
		...
		let service = new ConfigurationResolverService(uri.parse('file:///Essence/workspaceLocation'), envVariables, new TestEditorService(), TestEnvironmentService, configurationService, mockCommandService);
		if (platform.isWindows) {
			assert.strictEqual(service.resolve('abc ${config:editor.fontFamily} ${workspaceRoot} ${env:key1} xyz'), 'abc foo \\Essence\\workspaceLocation Value for Key1 xyz');
		} else {
			assert.strictEqual(service.resolve('abc ${config:editor.fontFamily} ${workspaceRoot} ${env:key1} xyz'), 'abc foo /Essence/workspaceLocation Value for Key1 xyz');
		}
	});
	...
	test('substitute many env variable and a configuration variable', () => {
		...
		let service = new ConfigurationResolverService(uri.parse('file:///Essence/workspaceLocation'), envVariables, new TestEditorService(), TestEnvironmentService, configurationService, mockCommandService);
		if (platform.isWindows) {
			assert.strictEqual(service.resolve('${config:editor.fontFamily} ${config:terminal.integrated.fontFamily} ${workspaceRoot} - ${workspaceRoot} ${env:key1} - ${env:key2}'), 'foo bar \\Essence\\workspaceLocation - \\Essence\\workspaceLocation Value for Key1 - Value for Key2');
		} else {
			assert.strictEqual(service.resolve('${config:editor.fontFamily} ${config:terminal.integrated.fontFamily} ${workspaceRoot} - ${workspaceRoot} ${env:key1} - ${env:key2}'), 'foo bar /Essence/workspaceLocation - /Essence/workspaceLocation Value for Key1 - Value for Key2');
		}
	});
